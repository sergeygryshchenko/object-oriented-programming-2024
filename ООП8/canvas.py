...
class TCanvasContent:
    ...
    def draw_content(self, canvas):
        ...
        active_vertex = self.graph.active_vertex
        p_size = 20
        ...
        for i in range(count):
            for j in range(count):
                if self.graph.get_adjacency_matrix_element(i,j) != 0:  
                    if i == j:
                        self.draw_loop(canvas, t[i], p_size)
                    else:
                        self.draw_lines(canvas, t, j, i, p_size)

        for i in range(count):
            x, y = t[i]
            color = "yellow" if i == active_vertex else "white"
            canvas.create_oval(x - p_size, y - p_size, x + p_size, y + p_size, fill=color, outline="black")
